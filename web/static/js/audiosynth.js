...
!function(){
	...
	setPub('generate', function(sound, note, octave, duration) {
		...
		if(typeof(this._fileCache[sound][octave-1][note][time])!='undefined') {
			...
			var waveBind = {modulate: this._mod, vars: this._temp, sound: thisSound};
			var val = 0;
			var curVol = 0;
...
Synth.loadSoundProfile({
	...
	wave: function(i, sampleRate, frequency, volume) {
		var o = this.sound.simpleWave.call(this, i, sampleRate, frequency, volume) 
		      + this.sound.simpleWave.call(this, i, sampleRate, frequency*0.5, volume);
		return clamp(o*10);
	}
},
{
